...
var browser = {
    ...
    orders: [],
    cms: "",
    dir: "media/uploads" + initDir()
};
browser.initDir = initDir
...
browser.openClipboard = function() {
    ...
    $.each(this.clipboard, function(i, val) {
        ...
        if (val.thumb)
            ...
            icon = '.';
        var icon = '/kcfinder/themes/' + browser.theme + '/img/files/small/' + icon + '.png';
        ...
        $.each(browser.files, function(i, file) {
            ...
            if (_.kuki.get('view') == 'list') {
                ...
                if (file.thumb)
                    ...
                    icon = '.';
                icon = '/kcfinder/themes/' + browser.theme + '/img/files/small/' + icon + '.png';
                html += '<tr class="file">' +
                ...
                if (file.thumb) {
                    // Patched by Avdeev Alexey, 13.06.2013
                    // var icon = browser.baseGetData('thumb') + '?file=' + encodeURIComponent(file.name) + '&dir=' + encodeURIComponent(browser.dir) + '&stamp=' + stamp;
                    var icon = browser.uploadURL +  '/' + browser.dir.replace(/^(\w+)/, "$1/.thumb") + '/' + encodeURIComponent(file.thumbName) + '?stamp=' + stamp;
                    ...
                    if (!icon.length) icon = '.';
                    icon = '/kcfinder/themes/' + browser.theme + '/img/files/big/' + icon + '.png';
                }
                html += '<div class="file">' +
...
browser.returnThumbnails = function(files) {
    if (this.opener.callBackMultiple) {
        ...
        $.each(files, function(i, file) {
            if ($(file).data('thumb')) {
                rfiles[j] = browser.thumbsURL + '/' + browser.dir.replace(/^(\w+)/, "$1/.thumb") + '/' + $(file).data('name');
...
browser.menuFile = function(file, e) {
    ...
    if (file.hasClass('selected') && files.length && (files.length > 1)) {
        ...
        if (data.thumb || data.smallThumb)
            html +='<a href="kcact:view">' + this.label("View") + '</a>';
        if (data.video)
            html +='<a href="kcact:view_video">' + this.label("View video") + '</a>';
        ...
        $('.menu a[href="kcact:pick_thumb"]').click(function() {
            var path = browser.thumbsURL + '/' + browser.dir.replace(/^(\w+)/, "$1/.thumb") + '/' + data.name;
    ...
    $('.menu a[href="kcact:view_video"]').on('click', function(e) {
        e.preventDefault();
        browser.hideDialog();
        $('#loading').html(browser.label("Loading video..."));
        $('#loading').css('display', 'inline');
        var ts = new Date().getTime();
        var url = _.escapeDirs(browser.uploadURL + '/' + browser.dir + '/' + data.name) + '?ts=' + ts;
        var $videoBlock = $('<div>', {
            id: 'kcfinderVideoPreview'
        });
        $('#dialog').empty().append($videoBlock);
        $videoBlock.nmdVideoPlayerJw({
            setup: {
                file: url,
                autostart: true,
                width: 525,
...
browser.buildTree = function(root, path) {
    ...
    var html = '<div class="folder' + (!root.writable ? ' readonly' : '') + '"><a href="kcdir:/' + _.escapeDirs(path) + '"><span class="brace">&nbsp;</span><span class="folder">' + _.htmlData(folderName) + '</span></a>';
    if (root.dirs) {
...
browser.changeDir = function(dir) {
    if (dir.children('span.folder').hasClass('regular')) {
        ...
        $.ajax({
            ...
            error: function() {
                $('#files').html(browser.label("Ошибка загрузки страницы"));
            }
        });
    }
};
...
browser.init = function() {
    browser.dir = "media/uploads" + initDir();
...
browser.resize = function() {
    ...
    _('left').style.height = _('right').style.height =
        $(window).height() - 14 - $('#status').outerHeight() + 'px'; // Patched by Avdeev Alexey
...
browser.fixFilesHeight = function() {
    _('files').style.height =
        ...
        (($('#search').css('display') != "none") ? $('#search').outerHeight() : 0) + 'px';
};
...
browser.fileNameDialog = function(e, post, inputName, inputValue, url, labels, callBack, selectAll) {
    if (url == browser.baseGetData('rename')) {
        // редактирование файла (хак)
        var html = '<form method="post" action="javascript:;">' +
            '<div class="box">' +
                '<label>Имя файла: <br><input name="' + inputName + '" type="text" placeholder="Имя файла" required></label><br />' +
                '<label>Заголовок: <br><input name="newTitle" type="text" placeholder="Заголовок" value="' + labels.properties.title + '"></label><br>' +
                '<label>Описание: <br><textarea name="newDesc" type="text" placeholder="Описание" rows="5">' + labels.properties.description + '</textarea></label><br>' +
                '<label>Ключевые слова (через запятую): <br><input name="newTags" type="text" placeholder="Ключевые слова (через запятую)" value="' + labels.properties.tags.join(', ') + '" /></label><br>' +
                '<div id="tagsContainer"></div>' +
                '<label>Авторы (через запятую): <br><input name="newAuthors" type="text" placeholder="Авторы (через запятую)" value="' + labels.properties.authors.join(', ') + '"></label><br>' +
        ...
        var html = '<form method="post" action="javascript:;">' +
            '<div class="box">' +
            '<input name="' + inputName + '" type="text" placeholder="Имя файла" /><br />' +
            '<div style="text-align:right">' +
            '<input type="submit" value="' + _.htmlValue(this.label("OK")) + '" /> ' +
            '<input type="button" value="' + _.htmlValue(this.label("Cancel")) + '" onclick="browser.hideDialog(); browser.hideAlert(); return false" />' +
        '</div></div></form>';
    ...
    $('#dialog input[name="' + inputName + '"]').attr('value', inputValue.replace(/\..+$/gi, ''));
    if (url == browser.baseGetData('rename') && !labels.writable) {
        $('#dialog input[name="' + inputName + '"]')
            .attr({
                disabled: true,
                title: 'Файл используется в системе, поэтому редактирование имени запрещено.'
            });
    }
    $('#dialog').unbind();
    $('#dialog').click(function() {
    ...
    $('#dialog form').submit(function() {
        ...
        // редактирование файла (хак)
        if (url == browser.baseGetData('rename')) {
            if (typeof post.newName != 'undefined') {
                post.newName = post.newName.replace(/\..+$/gi, '');    
            }
            post.newTitle = $(this).find('input[name="newTitle"]').val();
            post.newDesc = $(this).find('textarea[name="newDesc"]').val();
            post.newTags = $(this).find('input[name="newTags"]').val();
            post.newAuthors = $(this).find('input[name="newAuthors"]').val();
        }
        $.ajax({
            type: 'POST',
            dataType: 'json',
    ...
    $('#dialog input[name="newAuthors"]').autocomplete({
        serviceUrl: browser.baseGetData('authors_list'),
        minChars: 2,
        delimiter: /(,)\s*/,
        maxHeight: 120,
        deferRequestBy: 300,
    ...
    $('#dialog input[name="newTags"]').autocomplete({
        serviceUrl: browser.baseGetData('tags_list'),
        minChars: 2,
        delimiter: /(,)\s*/,
        maxHeight: 120,
        deferRequestBy: 300,
...
browser.baseGetData = function(act) {
    var data = '/backend/media/' + encodeURIComponent(this.type);
    if (act) {
        data += '/' + encodeURIComponent(act);
    }
    ...
    $('#searchForm').on('keyup', 'input[type="text"]', function(e){
        ...
        if (e.keyCode == 13) { // enter
            if (query.length >= 2) {
                ...
                $.ajax({
                    dataType: 'json',
                    url: browser.baseGetData('search'),
                    async: true,
                    method: 'get',
                    data: {
                        query: query
...
browser.initToolbar = function() {
    ...
    if (!_.kuki.isSet('displaySearch'))
        _.kuki.set('displaySearch', 'off');

    if (_.kuki.get('displaySearch') == 'on') {
        $('#toolbar a[href="kcact:search"]').addClass('selected');
        $('#search').css('display', 'block');
        browser.resize();
    }

    $('#toolbar a[href="kcact:search"]').click(function(){
        if ($('#search').css('display') == 'none') {
            $(this).addClass('selected');
            _.kuki.set('displaySearch', 'on');
            $('#search').css('display', 'block');
            browser.fixFilesHeight();
        } else {
            $(this).removeClass('selected');
